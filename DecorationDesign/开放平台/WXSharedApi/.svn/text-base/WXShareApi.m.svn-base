...
@implementation WXShareApi
@synthesize delegate;
@synthesize logindelegate;

static WXShareApi *weixin;
...
- (id)init
{
    if (self=[super init]) {
        //向微信注册
        [WXApi registerApp:@"wx74172e5eff8237ca" withDescription:@"demo 2.0"];
    }
    return self;
}
...
-(void) onResp:(BaseResp*)resp
{
    ...
    {
        NSString *strTitle = [NSString stringWithFormat:@"微信登陆"];
        NSString *strMsg = [NSString stringWithFormat:@"errcode:%d", resp.errCode];
        
        if (resp.errCode ==0) {
            NSLog(@"成功");
            
            NSString *url_str=[NSString stringWithFormat:@"https://api.weixin.qq.com/sns/oauth2/access_token?appid=wx74172e5eff8237ca&code=%@&secret=95e817ed0c5ba9ca10e695f78572635d&grant_type=authorization_code",[(SendAuthResp *)resp code]];
            ...
            [request setCompletionBlock:^{
                ...
                if (dic) {
                    ...
                    NSString *url_str=[PublicFunction urlDecode:[NSString stringWithFormat:@"https://api.weixin.qq.com/sns/userinfo?openid=%@&access_token=%@",[dic objectForKey:@"openid"],[dic objectForKey:@"access_token"]]];
                    ...
                    [request setCompletionBlock:^{
                        ...
                        if (dicc) {
                            ...
                            [logindelegate loginsuccessful:@"WXlogin" OAuth:dic userinfo:dicc];
                        }
                        else
                        {
                            NSLog(@"json解析错误!");
                            
                            [logindelegate loginfailed:@"WXlogin"];
                        }
                    }];
