...
- (id)init
{
    if ((self = [super init]))
    {
        ...
        memCache = [[NSMutableArray alloc] initWithCapacity:1];

        // Init the disk cache
//        NSArray *paths = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
        diskCachePath = [[[NSHomeDirectory() stringByAppendingPathComponent:@"Documents"] stringByAppendingPathComponent:@"ImageCache"] retain];  
...
- (void)notifyDelegate:(NSDictionary *)arguments
{
    ...
    if (image)
    {
        if ([memCache count]>50) {
            [memCache removeObjectAtIndex:0];
        }
        NSDictionary *lingshi=[[NSDictionary alloc]initWithObjectsAndKeys:image,@"image",key,@"key", nil];
        [memCache addObject:lingshi];
        [lingshi release];
...
- (UIImage *)imageFromKey:(NSString *)key fromDisk:(BOOL)fromDisk
{
    ...
    for (int i=0; i<[memCache count]; i++) {
        if ([[[memCache objectAtIndex:i]objectForKey:@"key"]isEqualToString:key]) {
            image=[[memCache objectAtIndex:i] objectForKey:@"image"];
        }
    }
...
- (void)queryDiskCacheForKey:(NSString *)key delegate:(id <SDImageCacheDelegate>)delegate userInfo:(NSDictionary *)info
{
    ...
    for (int i=0; i<[memCache count]; i++) {
        UIImage *image;
        NSString *cacheString =[[memCache objectAtIndex:i]objectForKey:@"key"];
        if (cacheString) {
            if ([[[memCache objectAtIndex:i]objectForKey:@"key"]isEqualToString:key]) {
                image=[[memCache objectAtIndex:i] objectForKey:@"image"];
...
- (void)removeImageForKey:(NSString *)key
{
    ...
    for (int i=0; i<[memCache count]; i++) {
        if ([[[memCache objectAtIndex:i]objectForKey:@"key"]isEqualToString:key]) {
            [memCache removeObjectAtIndex:i];
        }
    }
